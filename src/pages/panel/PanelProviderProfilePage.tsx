    ...
    return (
        <div className="flex-1 flex flex-col h-full overflow-hidden">
            ...
            <header className="sticky top-0 z-10 bg-background/80 backdrop-blur-md border-b border-border px-6 py-4 md:px-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-foreground tracking-tight">Perfil Técnico / Servicios</h2>
                    <p className="text-muted-foreground text-sm mt-1">Gestiona tu oferta de servicios y equipamiento.</p>
                </div>
                        ...
                        <div className="bg-card rounded-xl p-6 border border-border shadow-sm">
                            ...
                            <div className="grid grid-cols-1 gap-6">
                                <label className="flex flex-col gap-2">
                                    <span className="text-foreground text-sm font-medium">Nombre del Negocio / Profesional</span>
                                    <input
                                        type="text"
                                        value={businessName}
                                        onChange={(e) => { setBusinessName(e.target.value); setHasChanges(true); }}
                                        className="w-full bg-background border border-border rounded-lg px-4 py-3 text-foreground focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors placeholder:text-gray-600"
                                ...
                                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <label className="flex flex-col gap-2">
                                        <span className="text-foreground text-sm font-medium">Tipo de Perfil</span>
                                        <select
                                            value={providerType}
                                            onChange={(e) => { setProviderType(e.target.value as any); setHasChanges(true); }}
                                            className="w-full bg-background border border-border rounded-lg px-4 py-3 text-foreground focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors"
                                        >
                                ...
                                <label className="flex flex-col gap-2">
                                    <span className="text-foreground text-sm font-medium">Servicios Ofrecidos (separados por coma)</span>
                                    <div className="relative">
                                        <Wrench className="absolute left-3 top-1/2 -translate-y-1/2 text-muted-foreground" size={18} />
                                        <input
                                            type="text"
                                            value={servicesText}
                                            onChange={(e) => { setServicesText(e.target.value); setHasChanges(true); }}
                                            className="w-full bg-background border border-border rounded-lg pl-10 pr-4 py-3 text-foreground focus:outline-none focus:border-purple-500 focus:ring-1 focus:ring-purple-500 transition-colors placeholder:text-gray-600"
                                            placeholder="Ej. Sonido Directo, Iluminación, Backline, Grabación..."
                                        />
                                    </div>
                                    <p className="text-xs text-muted-foreground">Ej: Sonido, Luces, Montaje de Escenarios</p>
                                </label>
                            </div>
                        </div>
                    </section>
                ...
                <div className="fixed bottom-0 left-0 md:left-[280px] right-0 p-4 bg-background border-t border-border flex justify-between items-center z-30">
                    ...
                    <div className="flex items-center gap-3 ml-auto">
                        ...
                        <button
                            onClick={handleSave}
                            disabled={saving}
                            className="px-6 py-2.5 rounded-lg bg-purple-500 text-foreground text-sm font-bold hover:bg-purple-600 shadow-[0_0_15px_rgba(168,85,247,0.3)] transition-all flex items-center gap-2 disabled:opacity-50"
                        >
                            <Save size={16} />
