    ...
    if (error) {
        return (
            <div className="flex-1 flex flex-col items-center justify-center p-6 text-center">
                ...
                <h3 className="text-xl font-bold text-foreground mb-2">Hubo un problema</h3>
    ...
    return (
        <div className="flex-1 flex flex-col h-full overflow-hidden">
            ...
            <header className="sticky top-0 z-10 bg-background/80 backdrop-blur-md border-b border-border px-6 py-4 md:px-10 flex flex-col md:flex-row md:items-center justify-between gap-4">
                <div>
                    <h2 className="text-2xl font-bold text-foreground tracking-tight">Gestionar mi Perfil</h2>
                    <p className="text-muted-foreground text-sm mt-1">Configura tu identidad artística y presencia en Musikeeo.</p>
                </div>

                {/* Public View Toggle */}
                <div className="flex items-center gap-4 bg-card border border-border rounded-full px-4 py-2">
                    <div className="flex flex-col items-end mr-2">
                        <span className="text-xs font-bold text-foreground uppercase tracking-wider">Vista Pública</span>
                        <span className="text-[10px] text-muted-foreground">Haz visible tu perfil</span>
                    </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 md:p-10 space-y-8 pb-24">
                <div className="max-w-5xl mx-auto space-y-8">
                    ...
                    <section className="relative">
                        {/* Cover Image */}
                        <div className="relative w-full h-48 md:h-64 rounded-xl overflow-hidden bg-card border border-border">
                            <div
                            ...
                            <button
                                ...
                                className="absolute top-4 right-4 bg-black/60 hover:bg-black/80 text-foreground backdrop-blur-sm px-4 py-2 rounded-lg text-xs font-semibold flex items-center gap-2 transition-all border border-border"
                            >
                                <Camera size={14} />
                                Cambiar Portada
                            </button>
                        ...
                        <div className="px-4 md:px-8 -mt-16 relative z-10 flex items-end gap-6">
                            <div className="relative shrink-0">
                                <div
                                    className="size-32 md:size-40 rounded-full border-4 border-background bg-card bg-cover bg-center shadow-2xl"
                                    style={{
                            ...
                            <div className="hidden md:flex gap-2 mb-4">
                                {artist?.isVerified && (
                                    <span className="px-3 py-1 rounded-full bg-card border border-border text-xs font-medium text-muted-foreground flex items-center gap-1">
                    ...
                    <section className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                        ...
                        <div className="lg:col-span-2 space-y-6">
                            <div className="bg-card rounded-xl p-6 border border-border shadow-sm">
                                <h3 className="text-foreground text-lg font-bold mb-1 flex items-center gap-2">
                                    📋 Información Básica
                                </h3>
                                <p className="text-muted-foreground text-sm mb-6">Esta información aparecerá en la cabecera de tu perfil público.</p>

                                <div className="space-y-5">
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                                        <label className="flex flex-col gap-2">
                                            <span className="text-foreground text-sm font-medium">Nombre Artístico</span>
                                            <input
                                                type="text"
                                                value={artistName}
                                        ...
                                        <label className="flex flex-col gap-2">
                                            <span className="text-foreground text-sm font-medium">Ubicación</span>
                                    ...
                                    <label className="flex flex-col gap-2">
                                        <div className="flex justify-between">
                                            <span className="text-foreground text-sm font-medium">Biografía</span>
                                            <span className="text-muted-foreground text-xs">{bio.length}/500 caracteres</span>
                                        </div>
                                        <textarea
                                            value={bio}
                                    ...
                                    <div className="pt-2">
                                        <div className="flex flex-wrap gap-2 items-center">
                                            <span className="text-sm font-medium text-foreground mr-2 py-1">Géneros:</span>
                                            {genres.map((genre) => (
                                                <span
                                                    key={genre}
                                                    className="px-3 py-1 rounded-full bg-muted text-xs text-foreground border border-border flex items-center gap-1 group cursor-pointer hover:border-primary/50 transition-colors"
                        ...
                        <div className="lg:col-span-1 space-y-6">
                            ...
                            {/* Spotify */}
                            <div className="bg-card rounded-xl p-5 border border-border flex flex-col gap-4 shadow-sm hover:border-[#1DB954]/50 transition-colors">
                                ...
                                <div>
                                    <h4 className="text-foreground font-bold text-base">Spotify</h4>
                                    <p className="text-muted-foreground text-xs mt-1">Muestra tus canciones más populares.</p>
                                </div>
                                <button className="w-full mt-2 py-2 px-4 rounded-lg bg-muted text-foreground text-sm font-medium hover:bg-muted transition-colors flex items-center justify-center gap-2">
                                    Configurar
                                </button>
                            </div>

                            {/* Instagram */}
                            <div className="bg-card rounded-xl p-5 border border-border flex flex-col gap-4 shadow-sm hover:border-pink-500/50 transition-colors">
                                <div className="flex items-start justify-between">
