...
export const ArtistGridCard = ({ artist, type }: ArtistGridCardProps) => {
    ...
    return (
        <Link to={profileLink} className="group relative flex flex-col rounded-xl bg-card border border-border shadow-sm hover:shadow-lg hover:shadow-primary/5 transition-all duration-300 overflow-hidden hover:border-primary/50 cursor-pointer">
            {/* Image Container */}
            ...
            <div className="flex flex-1 flex-col p-4">
                ...
                <div className="flex flex-wrap gap-2 mb-4">
                    <span className="inline-flex items-center rounded bg-muted border border-border px-2 py-1 text-xs font-medium text-muted-foreground">
                        {roleOrGenre}
                    </span>
                    {/* Safely access genres/specialties */}
                    {(artist as any).genres?.slice(0, 2).map((g: string) => (
                        <span key={g} className="inline-flex items-center rounded bg-muted border border-border px-2 py-1 text-xs font-medium text-muted-foreground">
                            {g}
                        </span>
                    ))}
                </div>
                ...
                <div className="mt-auto flex items-center justify-between border-t border-border pt-3 text-sm">
                    <p className="text-muted-foreground text-xs">
                        Desde <span className="text-sm font-bold text-foreground">{getPriceDisplay()}</span> <span className="text-[10px]">/hora</span>
                    </p>
